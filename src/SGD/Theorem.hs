...
infixl 3 ?
{-@ (?) :: forall <pa :: Double -> Bool, pb :: () -> Bool>. Double<pa> -> ()<pb> -> Double<pa> @-}
(?) :: Double -> () -> Double 
x ? _ = x 
...
thm d zs1 ws1 as1@(SS α1 a1) f1 zs2 ws2 as2@(SS α2 a2) f2 
    ...
    =<= p * (dist (kant d) (bind sgd1 pureUpd1) (bind sgd2 pureUpd2))
            + q * (dist (kant d) (bind utail1 sgdRec1) (bind utail2 sgdRec2))
        ? pureUpdateEq z1 α1 f1
        ? pureUpdateEq z2 α2 f2

    =<= p * (dist (kant d) (bind sgd1 (ppure . update z1 α1 f1)) 
                           (bind sgd2 (ppure . update z2 α2 f2))) 
        + q * (dist (kant d) (bind utail1 sgdRec1) (bind utail2 sgdRec2))
        ? pureBindDist d d lip2α (update z1 α1 f1) sgd1 (update z2 α2 f2) sgd2 (relationalupdatep d z1 α1 f1 z2 α2 f2)
        ? assume (dist (kant d) (bind sgd1 (ppure . update z1 α1 f1)) 
                                (bind sgd2 (ppure . update z2 α2 f2))
                    <= dist (kant d) sgd1 sgd2 + lip2α)
    =<= p * (dist (kant d) sgd1 sgd2 + lip2α)
        + q * (dist (kant d) (bind utail1 sgdRec1) (bind utail2 sgdRec2))
        
         ? thm d zs1 ws1 a1 f1 zs2 ws2 a2 f2
         ? assume (dist (kant d) sgd1 sgd2 <= dist d ws1 ws2 + estab zs1 a1)
    *** Admit {-
    =<= p * (dist d ws1 ws2 + estab zs1 a1 + lip2α) -- ERROR
        + q * (dist (kant d) (bind utail1 sgdRec1) (bind utail2 sgdRec2))
        ...
        ? assert (0 <= q)
        ? multHelper (p * (dist d ws1 ws2 + estab zs1 a1 + lip2α)) q 
                 (dist (kant d) (bind utail1 sgdRec1) (bind utail2 sgdRec2))
                 (dist d ws1 ws2 + estab zs1 a1)
    =<= p * (dist d ws1 ws2 + estab zs1 a1 + lip2α) 
...
-}
 where
     lip2α = 2.0 * lip * α1
     sgd1 = sgd zs1 ws1 a1 f1
     sgd2 = sgd zs2 ws2 a2 f2
     p = one / lend zs1
     q = 1.0 - p
     uhead1 = ppure z1
     uhead2 = ppure z2
...
lemma d zs1 ws1 α1 a1 f1 zs2 ws2 α2 a2 f2 z = 
  dist (kant d) (sgdRecUpd zs1 ws1 α1 a1 f1 z) (sgdRecUpd zs2 ws2 α2 a2 f2 z)
    ...
    === dist (kant d) (bind sgd1 (pureUpdate z α1 f1)) 
                (bind sgd2 (pureUpdate z α2 f2))
        ?   pureBindDist d d 0 (update z α1 f1) sgd1
                             (update z α2 f2) sgd2 
                             (relationalupdateq d z α1 f1 z α2 f2)
    =<= dist (kant d) sgd1 sgd2
        ? thm d zs1 ws1 a1 f1 zs2 ws2 a2 f2
    =<= dist d ws1 ws2 + estab zs1 a1
    *** QED
    where 
